...
{
  $link = mysql_pconnect("localhost", "cecchet", "") or die ("ERROR: Could not connect to database: " + mysql_error());
  mysql_select_db("rubis", $link) or die("ERROR: Couldn't select RUBiS database: " + mysql_error($link));
}
...
{
  $result = mysql_query("SELECT id FROM users WHERE nickname=\"$nickname\" AND password=\"$password\"", $link) or die("ERROR: Authentification query failed: " + mysql_error($link));
