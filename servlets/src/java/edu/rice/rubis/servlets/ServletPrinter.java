...
public class ServletPrinter
{
	...
	void printCategory(String categoryName, int categoryId)
	{
		try
		{
			out.println("<a href=\"edu.rice.rubis.servlets.SearchItemsByCategory?category="
						+ categoryId
						+ "&categoryName="
	...
	void printCategoryByRegion(String categoryName, int categoryId, int regionId)
	{
		try
		{
			out.println("<a href=\"edu.rice.rubis.servlets.SearchItemsByRegion?category="
						+ categoryId
						+ "&categoryName="
						+ URLEncoder.encode(categoryName, "UTF-8")
						+ "&region="
	...
	void printCategoryToSellItem(String categoryName, int categoryId, int userId)
	{
		try
		{
			out.println("<a href=\"edu.rice.rubis.servlets.SellItemForm?category="
						+ categoryId
						+ "&user="
						+ userId
						+ "\">"
	...
	void printRegion(String regionName)
	{
		try
		{
			out.println("<a href=\"edu.rice.rubis.servlets.BrowseCategories?region="
	...
	void printItem(String itemName, int itemId, float maxBid, int nbOfBids, String endDate)
	{
		try
		{
			out.println("<TR><TD><a href=\"edu.rice.rubis.servlets.ViewItem?itemId="
						+ itemId
						+ "\">"
						+ itemName
						+ "<TD>"
	...
	*/
	void printItemDescription(int itemId,
							  String itemName,
							  String description,
							  float initialPrice,
							  float reservePrice,
							  ...
							  Connection conn)
	{
		...
		try
		{
			...
			if (userId > 0)
			{
				...
				this.printHTMLHighlighted("You are ready to bid on: " + itemName);
			}
			else
			{
			...
			if (buyNow > 0)
			{
				out.println("<p><a href=\"/rubis_servlets/servlet/edu.rice.rubis.servlets.BuyNowAuth?itemId="
							+ itemId
							+ "\">"
			...
			if (userId <= 0)
			{
				out.println("<a href=\"/rubis_servlets/servlet/edu.rice.rubis.servlets.PutBidAuth?itemId="
							+ itemId
			...
			if (userId > 0)
			{
				...
				this.printHTML("<form action=\"/rubis_servlets/servlet/edu.rice.rubis.servlets.StoreBid\" method=POST>\n"
	...
	void printItemUserHasBidOn(int itemId,
							   String itemName,
							   float initialPrice,
							   int quantity,
							   String startDate,
							   ...
							   String password)
	{
		try
		{
			out.println("<TR><TD><a href=\"/rubis_servlets/servlet/edu.rice.rubis.servlets.ViewItem?itemId="
						+ itemId
						+ "\">"
						+ itemName
						+ "<TD>"
	...
	void printUserBoughtItem(int itemId,
							String itemName,
							float buyNow,
							int quantity,
							int sellerId,
	...
	*/
	void printItemDescriptionToBuyNow(int itemId,
									  String itemName,
									  String description,
									  float buyNow,
									  int quantity,
									  ...
									  int userId)
	{
		try
		{
			...
			this.printHTMLHighlighted("You are ready to buy this item: " + itemName);
			...
			printHTML("<form action=\"/rubis_servlets/servlet/edu.rice.rubis.servlets.StoreBuyNow\" method=POST>\n"
					+ "<input type=hidden name=userId value="
					+ userId
					+ ">\n"
					+ "<input type=hidden name=itemId value="
	...
	void printBidHistory(int userId, String bidderName, float bid, String date)
	{
		try
		{
			out.println("<tr><td><a href=\"edu.rice.rubis.servlets.viewuserinfo?userid="
						+ userId
						+ "\">"
						+ bidderName
						+ "<TD>"
	...
	void printComment(String userName, int userId, String date, String comment)
	{
		try
		{
			out.println("<DT><b><BIG><a href=\"edu.rice.rubis.servlets.ViewUserInfo?userId="
						+ userId
						+ "\">"
						+ userName
						+ "</a></BIG></b>"
